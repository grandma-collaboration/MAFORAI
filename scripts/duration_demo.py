...
def main() -> int:
    chosen = _find_demo(DurationExtractor())
    if chosen is None:
        print("No real circular with an event duration was found in the search window.")
        return 1
...
def _find_demo(extractor: DurationExtractor) -> DemoChoice | None:
