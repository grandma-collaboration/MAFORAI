...
OUTPUT_PATH = (
    ...
    / "gcn"
    / "sweep"
    / "counterpart_association_audit.txt"
)
YEARS = (2023, 2024, 2025, 2026)
PER_YEAR = 4
MIN_SIGNALS = 2
ANCHORS = frozenset({34681})
...
def main() -> int:
    ...
    print(f"Wrote counterpart audit to {OUTPUT_PATH.relative_to(PROJECT_ROOT)}")
...
def select_audit_circulars(
    ...
    for source in circulars:
        ...
        score = len(COUNTERPART_SIGNAL_RE.findall(doc.rendered_text))
...
def render_audit(
    ...
    extractor = CounterpartAssociationExtractor()
    lines = [
        "=" * 100,
        "COUNTERPART_ASSOCIATION DEEP AUDIT",
        f"Selection: up to {PER_YEAR} circulars with at least {MIN_SIGNALS} "
        "counterpart-association signal hits per year plus fixed anchors",
        "=" * 100,
    ]
...
def _render_circular(
    ...
    if annotations:
        ...
        lines.append("  (no COUNTERPART_ASSOCIATION annotations)")

    lines.extend(["", "  --- lines containing counterpart-association signals ---"])
...
def _signal_lines(
    ...
    for raw_line in text.splitlines(keepends=True):
        ...
        matches = list(COUNTERPART_SIGNAL_RE.finditer(line))
        if matches:
            line_end = offset + len(raw_line)
            covered = any(
                annotation.span_start < line_end
                and offset < annotation.span_end
                for annotation in annotations
            )
            ...
            reason = (
                ""
                if covered
                else f" {_uncovered_reason(text, offset, matches)}"
            )
...
def _uncovered_reason(
    ...
    if "EVENT_IDENTITY" in deferrals:
        return "(defer: EVENT_IDENTITY)"
    return "(gap: review)"
