...
def main() -> int:
    ...
    if chosen is None:
        if first is None:
            ...
            return 1
        print("No recognizable event names found in the first 50 circulars; using the first one.")
        chosen = first

    circular, doc, annotations = chosen
    created_on = str(circular.get("created_on") or "")
    year = _year_from_created_on(created_on)
    year_status = "OK 2023+" if year is not None and year >= 2023 else "FAIL <2023"
