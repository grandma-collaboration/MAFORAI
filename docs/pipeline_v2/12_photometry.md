...
| `extraction_v2/photometry_tables.py` | Detects `TableBlock` objects, preserves block offsets and raw lines, recovers headers and surrounding context, splits rows, and infers `ColumnRole` values with confidence and optional time subtype. `emit_photometry_table_annotations()` can also represent a complete block as `PHOTOMETRY_TABLE` evidence. |
